...
gs_stagesurf_t * GL_MANGLING(device_stagesurface_create)(gs_device_t *device, uint32_t width,
...
void GL_MANGLING(gs_stagesurface_destroy)(gs_stagesurf_t *stagesurf)
{
	if (stagesurf) {
...
void GL_MANGLING(device_stage_texture)(gs_device_t *device, gs_stagesurf_t *dst,
		gs_texture_t *src)
{
...
uint32_t GL_MANGLING(gs_stagesurface_get_width)(const gs_stagesurf_t *stagesurf)
{
	return stagesurf->width;
}

uint32_t GL_MANGLING(gs_stagesurface_get_height)(const gs_stagesurf_t *stagesurf)
{
	return stagesurf->height;
}

enum gs_color_format GL_MANGLING(gs_stagesurface_get_color_format)(
		const gs_stagesurf_t *stagesurf)
{
	return stagesurf->format;
}

bool GL_MANGLING(gs_stagesurface_map)(gs_stagesurf_t *stagesurf, uint8_t **data,
		uint32_t *linesize)
{
...
void GL_MANGLING(gs_stagesurface_unmap)(gs_stagesurf_t *stagesurf)
{
	if (!gl_bind_buffer(GL_PIXEL_PACK_BUFFER, stagesurf->pack_buffer))
		return;
