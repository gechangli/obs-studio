		...
		struct shader_sampler *sampler)
{
	...
	new_sampler = GL_MANGLING(device_samplerstate_create)(shader->device, &info);
...
gs_shader_t * GL_MANGLING(device_vertexshader_create)(gs_device_t *device,
...
gs_shader_t * GL_MANGLING(device_pixelshader_create)(gs_device_t *device,
		const char *shader, const char *file,
		char **error_string)
{
	struct gs_shader *ptr;
...
void GL_MANGLING(gs_shader_destroy)(gs_shader_t *shader)
{
	size_t i;
...
int GL_MANGLING(gs_shader_get_num_params)(const gs_shader_t *shader)
{
	return (int)shader->params.num;
}

gs_sparam_t * GL_MANGLING(gs_shader_get_param_by_idx)(gs_shader_t *shader, uint32_t param)
{
	assert(param < shader->params.num);
	return shader->params.array+param;
}

gs_sparam_t * GL_MANGLING(gs_shader_get_param_by_name)(gs_shader_t *shader, const char *name)
{
	size_t i;
	for (i = 0; i < shader->params.num; i++) {
...
gs_sparam_t * GL_MANGLING(gs_shader_get_viewproj_matrix)(const gs_shader_t *shader)
{
	return shader->viewproj;
}

gs_sparam_t * GL_MANGLING(gs_shader_get_world_matrix)(const gs_shader_t *shader)
{
	return shader->world;
}

void GL_MANGLING(gs_shader_get_param_info)(const gs_sparam_t *param,
...
void GL_MANGLING(gs_shader_set_bool)(gs_sparam_t *param, bool val)
{
	int int_val = val;
	da_copy_array(param->cur_value, &int_val, sizeof(int_val));
}

void GL_MANGLING(gs_shader_set_float)(gs_sparam_t *param, float val)
{
	da_copy_array(param->cur_value, &val, sizeof(val));
}

void GL_MANGLING(gs_shader_set_int)(gs_sparam_t *param, int val)
{
	da_copy_array(param->cur_value, &val, sizeof(val));
}

void GL_MANGLING(gs_shader_set_matrix3)(gs_sparam_t *param, const struct matrix3 *val)
{
	struct matrix4 mat;
...
void GL_MANGLING(gs_shader_set_matrix4)(gs_sparam_t *param, const struct matrix4 *val)
{
	da_copy_array(param->cur_value, val, sizeof(*val));
}

void GL_MANGLING(gs_shader_set_vec2)(gs_sparam_t *param, const struct vec2 *val)
{
	da_copy_array(param->cur_value, val->ptr, sizeof(*val));
}

void GL_MANGLING(gs_shader_set_vec3)(gs_sparam_t *param, const struct vec3 *val)
{
	da_copy_array(param->cur_value, val->ptr, sizeof(*val));
}

void GL_MANGLING(gs_shader_set_vec4)(gs_sparam_t *param, const struct vec4 *val)
{
	da_copy_array(param->cur_value, val->ptr, sizeof(*val));
}

void GL_MANGLING(gs_shader_set_texture)(gs_sparam_t *param, gs_texture_t *val)
{
	param->texture = val;
}
		...
		struct program_param *pp)
{
	...
	if (pp->param->type == GS_SHADER_PARAM_BOOL ||
		...
		GL_MANGLING(device_load_texture)(program->device, pp->param->texture,
				pp->param->texture_id);
	}
}
...
void GL_MANGLING(gs_shader_set_val)(gs_sparam_t *param, const void *val, size_t size)
{
...
void GL_MANGLING(gs_shader_set_default)(gs_sparam_t *param)
{
	gs_shader_set_val(param, param->def_value.array, param->def_value.num);
}

void GL_MANGLING(gs_shader_set_next_sampler)(gs_sparam_t *param, gs_samplerstate_t *sampler)
{
